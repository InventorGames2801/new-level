...
class TestDatabasePerformance:
            ...
            try:
                ...
                for i in range(10):
                    ...
                    found_user = (
                        ...
                        .filter(User.email == f"thread{thread_id}user{i}@example.com")
                        .first()
                    )
    ...
    def test_large_result_set_performance(self, test_db_session):
        ...
        start_time = time.time()
        paginated_words = (
            test_db_session.query(Word)
            .filter(Word.difficulty == "easy")
            .offset(1000)
            .limit(100)
            .all()
        )
    ...
    def test_database_connection_performance(self, test_db_engine):
        ...
        ), f"Average connection time {avg_connection_time:.3f}s, expected < 0.1s"
        assert (
            max_connection_time < 0.5
        ), f"Max connection time {max_connection_time:.3f}s, expected < 0.5s"
