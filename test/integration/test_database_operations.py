...
from helpers import create_test_user, create_test_word
...
class TestDatabaseOperations:
    ...
    def test_update_user(self, test_db_session, sample_user):
        ...
        updated_user = database.update_user(
            test_db_session, sample_user.id, name="Updated Name", level=5
        )
    ...
    def test_update_user_password(self, test_db_session, sample_user):
        ...
        updated_user = database.update_user(test_db_session, sample_user.id, password="newpassword")
    ...
    def test_get_random_words(self, test_db_session, sample_user, sample_words):
        ...
        words = database.get_random_words(
            test_db_session, sample_user.id, count=2, difficulty="easy"
        )
    ...
    def test_get_random_words_with_exclusions(self, test_db_session, sample_user, sample_words):
        ...
        words = database.get_random_words(
            test_db_session, sample_user.id, count=5, excluded_ids=excluded_ids
        )
    ...
    def test_add_user_experience(self, test_db_session, sample_user, sample_game_settings):
        ...
        user, level_up, daily_limit_reached = database.add_user_experience(
            test_db_session, sample_user.id, 50  # Add 50 experience
        )
    ...
    def test_add_user_experience_level_up(self, test_db_session, sample_user, sample_game_settings):
        ...
        user, level_up, daily_limit_reached = database.add_user_experience(
            test_db_session, sample_user.id, 10  # Add 10 experience (should trigger level up)
        )

        assert level_up is True
        assert user.level == sample_user.level + 1

    def test_add_user_experience_daily_limit(
        self, test_db_session, sample_user, sample_game_settings
    ):
        ...
        user, level_up, daily_limit_reached = database.add_user_experience(
            test_db_session, sample_user.id, 20  # Would exceed daily limit
        )
    ...
    def test_create_game_session(self, test_db_session, sample_user):
        ...
        session = database.create_game_session(test_db_session, sample_user.id, "scramble")
    ...
    def test_database_transactions(self, test_db_session):
        ...
        user = User(name="Transaction Test", email="transaction@example.com", password_hash="hash")
        ...
        created_user = (
            test_db_session.query(User).filter(User.email == "transaction@example.com").first()
        )
    ...
    def test_database_constraints(self, test_db_session, sample_user):
        ...
        with pytest.raises(Exception):  # Should raise IntegrityError or similar
            duplicate_user = User(
                name="Duplicate", email=sample_user.email, password_hash="hash"  # Duplicate email
            )
            test_db_session.add(duplicate_user)
            test_db_session.commit()
    ...
    def test_cascading_deletes(self, test_db_session, sample_user):
        ...
        deleted_session = (
            test_db_session.query(GameSession).filter(GameSession.id == session_id).first()
        )
