    ...
    try:
        ...
        if user_id:
            try:
                ...
                if user:
                    return templates.TemplateResponse("index-log.html", {"request": request, "user": user})
...
def show_profile(request: Request, current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        return HTMLResponse(content="<h1>Ошибка при загрузке профиля</h1><p>Попробуйте позже</p>")
